...
{
    public class FactoryBasicServiceViewObj : FactoryBase
    {
        public FactoryBasicServiceViewObj(MappingSchema mappingSchema)
            : base(mappingSchema)
        {
        }
        ...
        protected override void WriteContent(StringWriter writer)
        {
            ...
            {
                if(!item.Key.EndsWith("DataService"))
                    continue;

                string className = item.Key.Substring(1, item.Key.Length - 12);//IACAccessLevelDataService
