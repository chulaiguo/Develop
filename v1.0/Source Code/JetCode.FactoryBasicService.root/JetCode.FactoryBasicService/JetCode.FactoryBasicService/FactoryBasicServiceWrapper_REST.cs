...
{
    public class FactoryBasicServiceWrapper_REST : FactoryBase
    {
        public FactoryBasicServiceWrapper_REST(MappingSchema mappingSchema)
            : base(mappingSchema)
        {
        }
        ...
        protected override void WriteContent(StringWriter writer)
        {
            ...
            foreach (KeyValuePair<string, Type> item in typeList)
            {
                ...
                foreach (MethodInfo info in list)
                {
                    ...
                    writer.WriteLine();
                    writer.WriteLine("\t\t\tHttpContent content = new ByteArrayContent(BasicServiceHelper.Serialize(_token_));");
                    ...
                    writer.WriteLine("\t\t\t{");
                    writer.WriteLine("\t\t\t\tthrow BasicServiceHelper.DeserializeException(res.Content.ReadAsByteArrayAsync().Result);");
                    writer.WriteLine("\t\t\t}");
                    writer.WriteLine();
        ...
        {
            writer.WriteLine("\tinternal static class BasicServiceHelper");
            writer.WriteLine("\t{");
