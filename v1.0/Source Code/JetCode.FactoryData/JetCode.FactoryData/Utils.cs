...
namespace JetCode.FactoryData
{
    public static class Utils
    {
        ...
        public static SortedList<string, Type> GetUtilsTypeList(string projectName)
        {
            SortedList<string, Type> retList = new SortedList<string, Type>();

            string path = string.Format(@"E:\Work\Projects\{0}\v{1:f1}\Bin\{0}.Utils.v{1:f1}.dll", projectName, GetLastestVersion(projectName));
            Assembly assembly = Assembly.LoadFrom(path);
            if (assembly != null)
            {
                Type[] typeList = assembly.GetTypes();
